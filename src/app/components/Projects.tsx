...
type Project = {
  ...
  city?: string;
  url: string;
  bannerSrc: string;  // Just use this for logo + banner combined
  tech: string[];
  cssStack?: string;
};
  ...
  return (
    ...
    >
      ...
      <p className="relative mx-auto mt-4 max-w-3xl text-center text-sm md:text-base opacity-90">
        Real websites we’ve shipped. Hover or focus to see the tech, client location, and a link to explore.
      </p>
        ...
        {projects.map((p, i) => (
          ...
          >
            ...
            <div className="relative aspect-[16/10] w-full overflow-hidden">
              ...
              {/* Overlay */}
              <div className="absolute inset-0 bg-black/80 opacity-0 transition-opacity duration-300 group-hover:opacity-100 group-focus-within:opacity-100 z-10" />

              {/* Overlay content */}
              <div className="absolute inset-x-0 bottom-0 p-4 md:p-5 opacity-0 translate-y-3 group-hover:opacity-100 group-hover:translate-y-0 group-focus-within:opacity-100 group-focus-within:translate-y-0 transition-all z-20">
                {/* Tech badges */}
                ...
                <div className="mt-3">
                  <Link
                    ...
                    className="inline-flex items-center gap-2 rounded-lg bg-white px-4 py-2 text-sm font-semibold text-black shadow-sm"
                  >
                    View Website
                    <svg
                      width="16"
